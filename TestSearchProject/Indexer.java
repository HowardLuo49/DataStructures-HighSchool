...
public class Indexer {
	...
	private void buildInvertedIndex(String filename) 
		...
		while(s.hasNext())
		{
			Tokenizer temp = new Tokenizer(" ", s.nextLine());
			String subject = null;
			while(temp.hasToken())
			{
				...
				{
					subject = temporary.substring(0, temporary.length()-1);
				}
				else if(!temporary.contains(":"))
				{
					KeyValue temporary2 = new KeyValue(temporary);
					temporary2.addValue(subject);
					table.insertKeyValue(index(temporary.toLowerCase()), temporary2);
				}
			}
		}
		s.close();
	...
	public KeyValue getKeyValue(String key) {
		...
		int num = index(key);
		return table.lookUpKey(num, key);
	...
	public int index(String key){
		...
		KeyValue temp = new KeyValue(key);
		return Math.abs(temp.hashCode()) % table.size();
