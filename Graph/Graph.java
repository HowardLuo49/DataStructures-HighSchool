...
public class Graph {
	...
	public Graph(String file) throws FileNotFoundException {
		...
		vertexList = new Vertex[sc.nextInt()];

		// read vertices
		for (int v=0; v < vertexList.length; v++) {
			vertexList[v] = new Vertex(sc.next(), null);
		}
		...
		while (sc.hasNext()) {
			...
			vertexList[v1].adjList = new Neighbor(v2, vertexList[v1].adjList);
			if (undirected) {
				// add v1 to front of v2's adjacency list
				vertexList[v2].adjList = new Neighbor(v1, vertexList[v2].adjList);
			}
		}
	}
	...
	private void dfs(int v, boolean[] visited) {
		visited[v] = true;
		System.out.println("\tvisiting " + vertexList[v].name);
		for (Neighbor e=vertexList[v].adjList; e != null; e=e.next) {
			if (!visited[e.vertexNum]) {
				System.out.println("\t" + vertexList[v].name + "--" + vertexList[e.vertexNum].name);
	...
	public int[] dfsTopsort() {
		 ...
		 int[] topseq = new int[vertexList.length];
	...
	public static void main(String[] args) 
		...
		if (file.equals("website2.txt")) {
			...
			for (int i=0; i < topseq.length; i++) {
				System.out.println(graph.vertexList[topseq[i]].name);
			}		 
		}
